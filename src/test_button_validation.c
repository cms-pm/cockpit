...
#define GPIOC_MODER     (GPIOC_BASE + 0x00)  // Mode register
#define GPIOC_PUPDR     (GPIOC_BASE + 0x0C)  // Pull-up/pull-down register  
#define GPIOC_IDR       (GPIOC_BASE + 0x10)  // Input data register
#define RCC_AHB2ENR     (0x40021000 + 0x4C)  // GPIO clock enable register
...
void run_test_button_validation_main(void) {
    ...
    record_register_state("RCC_AHB2ENR", RCC_AHB2ENR, true);
    record_register_state("GPIOC_MODER", GPIOC_MODER, true);
    record_register_state("GPIOC_PUPDR", GPIOC_PUPDR, true);
    record_register_state("GPIOC_IDR", GPIOC_IDR, true);
    ...
    record_register_state("RCC_AHB2ENR", RCC_AHB2ENR, false);
    record_register_state("GPIOC_MODER", GPIOC_MODER, false);
    record_register_state("GPIOC_PUPDR", GPIOC_PUPDR, false);
    record_register_state("GPIOC_IDR", GPIOC_IDR, false);
    
    // Step 4: Validate expected register configurations
    
    // RCC_AHB2ENR should have GPIOC clock enabled (bit 2 = 1)
    validate_register(RCC_AHB2ENR, GPIOC_CLOCK_BIT, GPIOC_CLOCK_BIT);
    
    // GPIOC_MODER should have PC13 configured as input (bits 26-27 = 00)
    validate_register(GPIOC_MODER, (0x3 << PC13_MODER_POS), (0x0 << PC13_MODER_POS));
    
    // GPIOC_PUPDR should have PC13 configured with pull-up (bits 26-27 = 01)
    validate_register(GPIOC_PUPDR, (0x3 << PC13_PUPDR_POS), (0x1 << PC13_PUPDR_POS));
