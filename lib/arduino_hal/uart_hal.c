...
// USART1 register addresses for direct access
...
#ifdef HARDWARE_PLATFORM
    debug_print("UART HAL: Initializing USART1 for USB-UART bridge");
...
void Serial_print(const char* str) {
    ...
    uart_write_string(str);     // Hardware UART (PA9/PA10)
...
void uart_debug_registers(void) {
    ...
    volatile uint32_t* rcc_apb2enr = (volatile uint32_t*)(STM32G4_RCC_BASE + STM32G4_RCC_APB2ENR_OFFSET);
    volatile uint32_t* gpioa_moder = (volatile uint32_t*)(STM32G4_GPIOA_BASE + STM32G4_GPIO_MODER_OFFSET);
    volatile uint32_t* gpioa_afr_high = (volatile uint32_t*)(STM32G4_GPIOA_BASE + 0x24);
    
    debug_print("UART Register Debug:");
    if ((*rcc_apb2enr) & STM32G4_RCC_APB2ENR_USART1EN) {
        debug_print("RCC_APB2ENR: USART1 clock enabled = YES");
    } else {
        debug_print("RCC_APB2ENR: USART1 clock enabled = NO");
    }
    debug_print("GPIOA_MODER: PA9/PA10 mode configuration");
    debug_print("GPIOA_AFRH: PA9/PA10 AF7 configuration");
